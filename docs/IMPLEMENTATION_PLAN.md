...
| **Milestones delivered** | M0, M1, M2, M3 (lib) ✅ · M5 abstraction + M7 core & plan→confirm UI + M8 runtime, WIT RFC & keybindings + M4 SFTP-mapping + M6 Docker- & K8s-mapping ✅ · M6-3 Docker logs streaming ✅ · **M6-6 complete**: K8s logs + exec + port-forward (RFC-0009 §1+§3) ✅ · cloud providers + live-transport (SSH/exec/K8s TUI pane) + LLM HTTP providers + WASM component bridge ⏭ · **v0.1.0 release prep** 🟡 (version `0.0.0`→`0.1.0`, CHANGELOG rolled to `[0.1.0] - 2026-06-30`, cross-platform release workflow ready; tagging is the maintainer's call) |
...
| M8-4 | Brokered creds/HTTP for plugins (UUID stand-in, host substitutes secret), journaled `Actor::Plugin` | plugin-systems-engineer, security-engineer | M8-2, M3-5 | rustdoc; threat-model; **RFC-0010** | plugin never sees secret value (test); brokered HTTP only | 🟡 **epoch deadline done**: `engine_config` enables `epoch_interruption`; an `EpochTicker` (background thread, weak engine ref, stops on drop) advances the engine epoch every 100 ms; each guest op re-arms a wall-clock deadline (`Limits::max_call_ticks`, default ≈5 s) via `PluginComponent::arm`, trapping a guest that **spins** past it as `Trap::Interrupt`→`PluginError::Timeout`→`VfsError::Backend{plugin_timeout}`. **WASI narrowing done (RFC-0010 PR-A)**: replaced blanket `add_to_linker_sync` with a per-interface allow-list in `crate::wasi_narrowing` — only `wasi:io/{error,streams,poll}`, `wasi:clocks/{wall-clock,monotonic-clock}`, and `wasi:random/*` are registered; sockets, filesystem, and CLI are absent (default-deny). `wasi:io/poll` and `wasi:clocks/monotonic-clock` use non-blocking stubs (`ImmediatelyReady` pollable) that close the epoch-vs-blocking-WASI evasion gap. Fixture guest rebuilt as `#![no_std]` + `dlmalloc` so `backend.wasm` imports no `wasi:cli/*`. All 44 tests green. **Design complete: RFC-0010** (plugin sandbox hardening + brokered host fns) specifies the WASI allow-list (remove sockets/filesystem/CLI/env; non-blocking poll stub), blocking-evasion closure, real `http-fetch` (reqwest/rustls, SSRF guards, size+time limits), `use-credential` closed action vocabulary (`bearer-token`/`basic-auth-header`/`sign-aws-request`), `CredentialBroker` trait split, and dep-closure CI extension. **Remaining (phased per RFC-0010)**: PR-B1 real http-fetch (behind `plugin-network` feature, env-gated integration test); PR-B2 real use-credential (mock broker test + dep-closure extension) |
