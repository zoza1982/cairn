...
| M2-5 | Transfer queue UI overlay: per-item progress/speed/ETA, reorder, controls | tui-engineer | M2-2, M1-6 | user docs | **Demo:** copy a big tree local→local with live queue | 🟡 live byte-progress (#40) + **sequential FIFO queue** (#45): a transfer issued while one runs is queued and auto-started on completion; status shows queue depth. Dedicated queue overlay (per-item progress/speed/ETA, reorder) + concurrent transfers + pause/resume deferred |
