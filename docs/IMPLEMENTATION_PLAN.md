...
| M2-5 | Transfer queue UI overlay: per-item progress/speed/ETA, reorder, controls | tui-engineer | M2-2, M1-6 | user docs | **Demo:** copy a big tree local→local with live queue | 🟡 live byte-progress (#40) + **sequential FIFO queue** (#45) + **queue view overlay** (#46, `Ctrl-T`) + **throughput rate** (#48) + **queue controls** (#50, cursor + drop-selected): a transfer issued while one runs is queued and auto-started; status shows bytes, rate, and queue depth; the queue view can drop a specific pending transfer. Per-item progress + ETA + reorder + concurrent transfers + pause/resume deferred |
