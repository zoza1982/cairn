...
| M2-5 | Transfer queue UI overlay: per-item progress/speed/ETA, reorder, controls | tui-engineer | M2-2, M1-6 | user docs | **Demo:** copy a big tree local→local with live queue | 🟡 **live byte-progress in the status line done** (#40): the engine's progress callback feeds a coalesced, non-blocking `TransferProgress`/`TransferDone` pair; one transfer at a time. Full queue overlay (per-item progress/speed/ETA/reorder, concurrent queue + pause/resume) deferred |
