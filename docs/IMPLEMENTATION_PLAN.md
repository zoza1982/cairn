...
| **Milestones delivered** | M0, M1, M2, M3 (lib) ✅ · M5 abstraction + M7 core & plan→confirm UI + M8 runtime, WIT RFC & keybindings + M4 SFTP-mapping + M6 Docker- & K8s-mapping ✅ · M6-3 Docker logs streaming ✅ · M6-6 K8s logs streaming + **K8s interactive exec** (RFC-0009 §1+§3) 🟡 · cloud providers + live-transport (SSH/exec/K8s port-forward) + LLM HTTP providers + WASM component bridge ⏭ · **v0.1.0 release prep** 🟡 (version `0.0.0`→`0.1.0`, CHANGELOG rolled to `[0.1.0] - 2026-06-30`, cross-platform release workflow ready; tagging is the maintainer's call) |
...
| M6-3 | Docker actions: `exec`, `logs` (Stream), start/stop | container-backend-engineer | M6-2 | rustdoc | exec interactive stream; logs follow | 🟡 **logs streaming landed**: `DockerVfs::invoke("logs")` returns `ActionOutcome::Stream` backed by bollard's `Docker::logs` endpoint — bollard `LogOutput` demuxes the Docker 8-byte stream header; a Tokio spawn+mpsc-channel pattern produces a `'static` `BoxStream` with natural cancellation. `follow`/`tail` from `ActionCtx::Logs`; defaults `follow:false/tail:100` so dind tests terminate. `ContainerOps::logs` seam + `MockContainerOps` + 4 hermetic tests + 1 dind integration test. **Remaining:** `exec` (interactive bidirectional Session, bollard exec+websocket attach) + start/stop. K8s streaming = M6-6 |
...
| M6-6 | K8s streaming: `logs(follow)` Stream, `exec` (tty Session), `port-forward` | kube-staff-engineer | M6-5b | rustdoc; RFC-0009 | cp out of pod completes (no stall); port-forward holds | 🟡 **logs + exec landed**: `KubeVfs::invoke("logs")` → `ActionOutcome::Stream` (mpsc+spawn over `log_stream`); `KubeVfs::invoke("exec")` with `ActionCtx::Exec` → `ActionOutcome::Session(SessionHandle)` via `Api::<Pod>::exec` + `AttachParams`; `AttachedProcess` stdin/stdout/stderr wired to `SessionHandle` channels via relay tasks; TTY resize relay uses `terminal_size()` + `try_send`; `done` carries `Result<i32, VfsError>` exit code (RFC-0009 §1); `SessionHandle` refined: `resize` field added, `SessionHandle::new()` constructor added; `MockKube::exec` echo-style + 5 hermetic unit tests + kind integration test (`k8s_exec_non_tty_round_trip`, `CAIRN_IT_K8S`-gated). Workspace clippy + rustdoc clean. **Remaining M6-6:** `port-forward` (`Portforwarder`-based `Session`, RFC-0009 §3); TUI exec pane = M6-7. |
| M6-7 | Stream/Session UI: live log viewer (follow+filter), exec pane, port-forward status | tui-engineer | M6-3, M6-6 | user docs; RFC-0009 | **Demo:** stream pod logs; exec; copy pod→S3 | 🟡 **stream half landed** (M6-7-core): reducer (`LogViewer` overlay, `LogViewerStatus`, ring-buffer 100k/4MiB, follow mode, scroll), `AppEffect::OpenLogViewer`/`CloseLogViewer`, `AppEvent::LogChunk`/`LogStreamEnded`, keybindings (`L`, `PageUp`, `PageDown`), `render_log_viewer`, app wiring (`CancellationToken` per session, real `Vfs::invoke("logs")` follow stream → `AppEvent::LogChunk`; backends without log streaming surface `VfsError::Unsupported`). **Remaining:** in-viewer filter, exec pane, port-forward status. |
...
| M8-4 | Brokered creds/HTTP for plugins (UUID stand-in, host substitutes secret), journaled `Actor::Plugin` | plugin-systems-engineer, security-engineer | M8-2, M3-5 | rustdoc; threat-model; **RFC-0010** | plugin never sees secret value (test); brokered HTTP only | 🟡 **epoch deadline done**: `engine_config` enables `epoch_interruption`; an `EpochTicker` (background thread, weak engine ref, stops on drop) advances the engine epoch every 100 ms; each guest op re-arms a wall-clock deadline (`Limits::max_call_ticks`, default ≈5 s) via `PluginComponent::arm`, trapping a guest that **spins** past it as `Trap::Interrupt`→`PluginError::Timeout`→`VfsError::Backend{plugin_timeout}`. Caveat (gate-caught): epoch only interrupts guest wasm, not a guest *blocked inside a host/WASI call* (the full WASI surface includes a blocking `wasi:io/poll`) — narrowing the linked WASI subset is owed before live untrusted use (M8-5). **Design complete: RFC-0010** (plugin sandbox hardening + brokered host fns) specifies the WASI allow-list (remove sockets/filesystem/CLI/env; non-blocking poll stub), blocking-evasion closure, real `http-fetch` (reqwest/rustls, SSRF guards, size+time limits), `use-credential` closed action vocabulary (`bearer-token`/`basic-auth-header`/`sign-aws-request`), `CredentialBroker` trait split, and dep-closure CI extension. **Remaining (phased per RFC-0010)**: PR-A WASI narrowing; PR-B1 real http-fetch (behind `plugin-network` feature, env-gated integration test); PR-B2 real use-credential (mock broker test + dep-closure extension) |
| M8-5 | Manifest (`plugin.toml` + wasm section), install-time capability approval UI, revocation | plugin-systems-engineer, tui-engineer | M8-2, M8-4 | user docs; plugin author guide; **RFC-0010** | **Demo:** install a sample plugin backend, approve caps, browse | 🟡 **Design complete: RFC-0010 §5** specifies `plugin.toml` schema (deny-unknown-fields, api major, capabilities, limits), directory discovery, ABI compatibility checks, grants storage in cairn-config, install-time approval flow (PR-C1 loader + PR-C2 TUI approval overlay + PR-C3 install subcommand). **Remaining**: implement per RFC-0010 phased plan (PR-C1/C2/C3) |
