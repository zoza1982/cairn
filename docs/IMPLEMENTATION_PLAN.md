...
| M6-6 | K8s streaming: `logs(follow)` Stream, `exec` (tty Session), `port-forward` | kube-staff-engineer | M6-5b | rustdoc | cp out of pod completes (no stall); port-forward holds | 🟡 **logs streaming landed**: `KubeVfs::invoke("logs")` returns `ActionOutcome::Stream` — `KubeOps::logs` seam + `KubeRsOps` implementation over `kube 4.0` `Api::<Pod>::log_stream` (spawn+mpsc pattern, `Box::pin` for non-Unpin `AsyncBufRead`, `follow`/`tail` from `ActionCtx::Logs`); `MockKube::logs` (canned 2-line output); 5 hermetic unit tests; kind integration test extended (non-follow, tail 10, asserts no stream errors). `bytes` unconditional dep; `tokio/rt`+`tokio/sync` added to `k8s` feature. **Remaining M6-6 follow-ups:** `exec` (interactive TTY `Session`, WebSocket attach) + `port-forward` (`Portforwarder`-based `Session`). |
| M6-7 | Stream/Session UI: live log viewer (follow+filter), exec pane, port-forward status | tui-engineer | M6-3, M6-6 | user docs | **Demo:** stream pod logs; exec; copy pod→S3 | 🟡 **stream half landed** (M6-7-core): reducer (`LogViewer` overlay, `LogViewerStatus`, ring-buffer 100k/4MiB, follow mode, scroll), `AppEffect::OpenLogViewer`/`CloseLogViewer`, `AppEvent::LogChunk`/`LogStreamEnded`, keybindings (`L`, `PageUp`, `PageDown`), `render_log_viewer`, app wiring (`CancellationToken` per session, real `Vfs::invoke("logs")` follow stream → `AppEvent::LogChunk`; backends without log streaming surface `VfsError::Unsupported`). **Remaining:** in-viewer filter, exec pane, port-forward status. |
