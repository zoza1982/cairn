...
| M6-6 | K8s streaming: `logs(follow)` Stream, `exec` (tty Session), `port-forward` | kube-staff-engineer | M6-5b | rustdoc | cp out of pod completes (no stall); port-forward holds | 🟡 action surface (#27): `actions_at` advertises pod `logs`/`port-forward` and container `logs`/`exec` by depth, mock-tested; live streams/sessions + port-forward = integration step. **API ready (#33):** `Vfs::invoke` takes the target `path` + `ActionOutcome::Session`/`SessionHandle` added (RFC-0007 Gap 1); only the live kube streams/sessions remain |
