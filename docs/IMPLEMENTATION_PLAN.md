...
| **Milestones delivered** | M0, M1, M2, M3 (lib) ✅ · M5 abstraction + M7 core & plan→confirm UI + M8 runtime, WIT RFC & keybindings + M4 SFTP-mapping + M6 Docker- & K8s-mapping ✅ · M6-3 Docker logs streaming ✅ · M6-6 K8s logs streaming (first slice) 🟡 · cloud providers + live-transport (SSH/exec/K8s exec+port-forward) + LLM HTTP providers + WASM component bridge ⏭ · **v0.1.0 release prep** 🟡 (version `0.0.0`→`0.1.0`, CHANGELOG rolled to `[0.1.0] - 2026-06-30`, cross-platform release workflow ready; tagging is the maintainer's call) |
...
| M6-6 | K8s streaming: `logs(follow)` Stream, `exec` (tty Session), `port-forward` | kube-staff-engineer | M6-5b | rustdoc; RFC-0009 | cp out of pod completes (no stall); port-forward holds | 🟡 **logs streaming landed**: `KubeVfs::invoke("logs")` returns `ActionOutcome::Stream` — `KubeOps::logs` seam + `KubeRsOps` implementation over `kube 4.0` `Api::<Pod>::log_stream` (spawn+mpsc pattern, `Box::pin` for non-Unpin `AsyncBufRead`, `follow`/`tail` from `ActionCtx::Logs`); `MockKube::logs` (canned 2-line output); 5 hermetic unit tests; kind integration test extended (non-follow, tail 10, asserts no stream errors). `bytes` unconditional dep; `tokio/rt`+`tokio/sync` added to `k8s` feature. **Remaining M6-6 follow-ups:** `exec` (interactive TTY `Session`, WebSocket attach) + `port-forward` (`Portforwarder`-based `Session`). |
